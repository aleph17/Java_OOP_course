...
public class Board {
    ...
    */
    ...
    public String visualize(Generation generation) {
        ...
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                Cell c = getTile(new Coord(x, y)).getCell();

                if (!generation.getCellAlivenessStates().get(c)) {
                    sb.append('0');
                } else {
                    switch (c.getType()) {
    ...
    public Cell getHighestEnergyCell(Generation gen) {
        return  gen.getAliveCells().stream()
                .max(Comparator.comparing(Cell::getLifePoints)
                        .thenComparing(Cell::getY, Comparator.reverseOrder())
                        .thenComparing(Cell::getX, Comparator.reverseOrder())).orElse(null);
    }
    ...
    public Map<Integer, List<Cell>> getCellsByEnergyLevel(Generation gen) {
        return  gen.getMapLifePoints().entrySet().stream()
    ...
    public List<Cell> topEnergyCells(Generation gen, int n) {
        return gen.getAliveCells().stream().sorted(Comparator.comparing(Cell::getLifePoints).reversed())
    ...
    public Map<Integer, List<Cell>> groupByAliveNeighborCount(Generation gen) {
        return gen.getAliveCells().stream()
                .collect(Collectors.groupingBy(Cell::countAliveNeighbors,Collectors.toList()));
    }
    ...
    public IntSummaryStatistics energyStatistics(Generation gen) {
        return gen.getAliveCells().stream().collect(Collectors.summarizingInt(Cell::getLifePoints));
    }
    ...
    public Map<Integer, IntSummaryStatistics> getTimeSeriesStats(int fromStep, int toStep) {
        Map<Integer, IntSummaryStatistics> mapper = new HashMap<>();
        ...
        for (int step = fromStep; step <= toStep; step++) {
            if (step >= 0 && step < generations.size()) {
                ...
                IntSummaryStatistics stats = gen.getMapLifePoints().entrySet().stream()
                        .filter(e -> gen.getCellAlivenessStates().get(e.getKey()))
                        .collect(Collectors.summarizingInt(Map.Entry::getValue));
                mapper.put(step, stats);
            }
        }
        return mapper;
    }
