...
public class Cell implements Evolvable, Interactable {
    ...
    public void interact(Cell otherCell) {
        ...
        CellMood m1 = this.cellMood;
        CellMood m2 = otherCell.cellMood;

        if(m1==CellMood.HEALER && m2==CellMood.NAIVE){
            otherCell.lifepoints++;
        }
        else if(m1==CellMood.HEALER && m2==CellMood.VAMPIRE){
            if(this.lifepoints>=0){
                otherCell.lifepoints++;
                this.lifepoints--;   
            }
        }
        else if((m1==CellMood.VAMPIRE && m2==CellMood.NAIVE)){
            if(otherCell.lifepoints>=0){
                this.lifepoints++;
                otherCell.lifepoints--;
            }
            otherCell.cellMood = CellMood.VAMPIRE;
        }
        if (m1 == CellMood.NAIVE && m2 == CellMood.VAMPIRE) {
            if (this.lifepoints >= 0) {
                otherCell.lifepoints++;
                this.lifepoints--;
            }
            this.cellMood = CellMood.VAMPIRE;
             ...
             //in all other cases, nothing happen 
        }
