...
public class Game {
    ...
    Tile tile = board.getTile(new Coord(cell.getX(), cell.getY()));
     int oldModifier = tile.getLifePointModifier();
     switch (event) {
            case CATACLYSM:
                tile.setLifePointModifier(-cell.getLifePoints());
                tile.interact(cell);
                break;
            case FAMINE:
            ...
            case BLOOD_MOON:
                if (cell.getMood() == CellMood.VAMPIRE) {
                    for (Tile neighbor : cell.getNeighbors()) {
                        Cell target = neighbor.getCell();
                        if (target.getMood() == CellMood.HEALER) {
            ...
            case SANCTUARY:
                ...
                } else if (cell.getMood() == CellMood.HEALER) {
                    tile.setLifePointModifier(1);
                    tile.interact(cell);
                }
                break;
        ...
        // Restore the original modifier after the switch
        tile.setLifePointModifier(oldModifier);   
    ...
    public void setMood(CellMood mood, List<Coord> targetCoordinates) {
          if (mood == null || targetCoordinates == null) return;
    ...
    public void setMoods(CellMood mood, List<Coord> coordinates) {
        if (coordinates == null || mood == null)
           //setMood(mood, coordinates);
            return;
    ...
    public static Map<Integer, EventType> loadEvents(Game game) {
        ...
        return new HashMap<>(game.getEventMapInternal());
    }
}
