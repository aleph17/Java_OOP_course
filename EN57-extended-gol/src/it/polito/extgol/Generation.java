...
public class Generation {
    ...
    @ElementCollection(fetch = FetchType.LAZY)
    @CollectionTable(name = "generation_mood", joinColumns = {
        @JoinColumn(name = "generation_id", referencedColumnName = "id"),
        @JoinColumn(name = "game_id", referencedColumnName = "game_id"),
        @JoinColumn(name = "board_id", referencedColumnName = "board_id")
    })
    ...
    private Map<Cell, CellMood> cellMoodStates = new HashMap<>();
     ...
     */
    @ElementCollection(fetch = FetchType.LAZY)
    @CollectionTable(name = "generation_points", joinColumns = {
            // This maps generation_state.generation_id → generation.id
            @JoinColumn(name = "generation_id", referencedColumnName = "id"),
            // These map generation_state.game_id → generation.game_id
            // generation_state.board_id → generation.board_id
    ...
    @MapKeyJoinColumn(name = "cell_id")
    @Column(name = "has_points", nullable = false)
    ...
    public void setType(List<Coord> coords, CellType type) {
        for(Coord c: coords) {
            this.board.getTile(c).getCell().setType(type);
        }
    }
...
//        next.snapLifePoints();
    ...
    /**
     * Captures the current LifePoints of every cell on the board
     * into the persistent cellLifePoints map and returns an unmodifiable
     * snapshot.
     *
     * Iterates over each Tile in the associated Board, validates that a Cell
     * exists on the tile, and records its isAlive value. After clearing any
     ...
     * @throws ExtendedGameOfLifeException if any Tile does not contain a Cell
     */
    public Map<Cell, Integer> snapLifePoints() {
        ...
        return Map.copyOf(cellLifePoints);
    }

    public Map<Cell, CellMood> snapMoods() {
    cellMoodStates.clear();
    for (Tile t : board.getTiles()) {
        Cell c = t.getCell();
        cellMoodStates.put(c, c.getMood());
    }
    return Map.copyOf(cellMoodStates);
}
    ...
    public static Generation createInitial(Game game, Board board, Map<Coord, CellType> cellTypesMap) {
        ...
        for(Map.Entry<Coord, CellType> entry : cellTypesMap.entrySet()){
            ...
            switch(type){
                case HIGHLANDER -> cell = new Highlander(coord);
                case LONER -> cell = new Loner(coord);
                case SOCIAL -> cell = new Social(coord);
                default -> cell = new Cell(coord);
            }

            cell.setType(type);
            cell.setAlive(true);
            cell.setTile(board.getTile(coord));
            board.getTile(coord).setCell(cell);
        }
    ...
    public Map<Cell, Integer> getMapLifePoints(){
        return this.cellLifePoints;
    }
}
